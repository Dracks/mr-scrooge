...
export class DemoDataService {
    ...
    async generateTagAndTransactions(groupOwnerId: number, label: string, amountRange: Range, periodicity: "days" | 'montly'){
        ...
        while (isBefore(finish, date)){
            const transaction = await this.transactionService.addTransaction({date: new DateOnly(date).toString(), groupOwnerId, kind: 'demo', movementName: `translaction ${label}`, value: getAmount(), pageKey: ''})
    ...
    async generateAll(groupOwnerId: number){
        ...
        const labelsList = await Promise.all(Object.keys(labelConfigMap).map(async (label) => {
