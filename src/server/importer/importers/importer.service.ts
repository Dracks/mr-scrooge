...
export interface ParserFactory {
    create(
        filePath: string,
    ): Iterable<BankTransactionBase> | Promise<Iterable<BankTransactionBase>> | AsyncGenerator<BankTransactionBase>;
...
export class NewImportService {
    ...
    async import(groupOwnerId: number, key: string, fileName: string, filePath: string) {
        ...
        try {
            ...
            const source = await ensureOrThrow(
                this.parsersMap.get(key),
                new Exception('E10003', 'Parser not found', { parserKey: key }),
            ).create(filePath);
            let discarting = true;
            ...
            for await (const transaction of source) {
                ...
                if (await this.bankTransactionService.existsSimilar(groupOwnerId, key, transaction)) {
                    ...
                    if (previous && previousState) {
                        const record = await this.bankTransactionService.addTransaction({
                            kind: key,
                            groupOwnerId,
                            ...previous,
                        });
