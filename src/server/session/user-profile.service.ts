...
export class UserProfileService{
    ...
    constructor(@InjectModel(UserModel) private readonly userModel: typeof UserModel, readonly passwordService: PasswordService){}

    public async getUserProfile(userId: number): Promise<UserModelAttributes| undefined> {
    ...
    public async validateUser(username: string, password: string): Promise<UserModelAttributes | undefined> {
        this.logger.log( {username, password}, 'validateUser');
        const users = await this.userModel.findAll({where: Sequelize.and(Sequelize.or({username}, {email: username}), {isActive: true})});
    ...
    public async addUser(username: string, password: string, options: Partial<Omit<UserModel, 'username' | 'password'>>={}): Promise<UserModelAttributes> {
