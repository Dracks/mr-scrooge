...
export class GraphResolver{
    ...
    async validateGraph<T extends NewGraph>(groupsId: number[], graph: T): Promise<GQLBaseError | undefined> {
        ...
        const allLabels = [graph.labelFilter, ...(graph.horizontalGroup?.labels??[]), ...(graph.group?.labels ?? []) ].filter(Boolean) as number[]
    ...
    @Query(()=>[Graph])
    async graphs(@GqlGroupsId() groupsId: number[], @Args('graphsIds', {type: ()=>[Int], nullable: true}) graphIds: number[]){
