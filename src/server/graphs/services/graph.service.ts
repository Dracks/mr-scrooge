...
import { GraphModel, GraphGroupModel, GraphGroupTagsModel, GraphHorizontalGroupModel, GraphHorizontalGroupTagsModel } from "../models/graph.model";
...
class GraphBuilder {
    ...
    setHorizontalGroup(
        horizontalGroup: InferAttributes<GraphHorizontalGroupModel>
    ) {
...
export class GraphService {
    constructor(
        ...
        @InjectModel(GraphHorizontalGroupModel) private readonly graphHorizontalGroupModel: typeof GraphHorizontalGroupModel,
        @InjectModel(GraphHorizontalGroupTagsModel) private readonly graphHorizontalGroupTagsModel: typeof GraphHorizontalGroupTagsModel,
    ) { }
    ...
    async createGraph(
        { group: { tags: groupTags, ...group }, horizontalGroup, ...graph }: NewGraph,
    ) {
        ...
        if (groupTags) {
            ...
            const groupTagsData = await Promise.all(groupTags.map(tag => this.graphGroupTagsModel.create({ groupId, labelId: tag })))
        ...
        if (horizontalGroup) {
            ...
            if (horizontalTags) {
                const groupId = groupData.dataValues.id
                const horizontalTagsData = await Promise.all(horizontalTags.map(tag => this.graphHorizontalGroupTagsModel.create({ groupId, labelId: tag })))
