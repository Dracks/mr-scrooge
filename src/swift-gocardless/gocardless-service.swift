...
class GoCardlessService {
    ...
    init(client: HTTPClient, baseUrl: String = "https://bankaccountdata.gocardless.com/api/v2", secretId: String, secretKey: String) {
        self.client = client
    ...
    func obtainTokens() async throws {
        ...
        let request = try await createRequest(path: "/token/new/", method: .POST, body: body)
        ...
        guard response.status.code == 200 else {
            if response.status.code == 401 {
                ...
                let responseBuffer = try await response.body.collect(upTo: 1024 * 1024) // 1MB limit
                let responseBody = String(buffer: responseBuffer)
                throw GoCardlessError.apiError("HTTP \(response.status.code): \(responseBody)")
            }
        }
        ...
        let tokenResponse = try decoder.decode(GoCardlessTokenResponse.self, from: Data(buffer.readableBytesView))
        ...
        let expirationTime = Date().addingTimeInterval(TimeInterval(tokenResponse.accessExpires))
    ...
    func refreshAccessToken() async throws {
        ...
        let body = try JSONSerialization.data(withJSONObject: payload)
        let request = try await createRequest(path: "/token/refresh/", method: .POST, body: body)
    ...
    func getInstitutions(country: String? = nil) async throws -> [GoCardlessInstitution] {
        ...
        let request = try await createRequest(path: "/institutions/\(queryString)", method: .GET)
        let response = try await client.execute(request, timeout: .seconds(30))
        ...
        let decoder = JSONDecoder()
        let institutions = try decoder.decode([GoCardlessInstitution].self, from: Data(buffer.readableBytesView))
    ...
    func getInstitution(id: String) async throws -> GoCardlessInstitution {
        ...
        guard response.status.code == 200 else {
            if response.status.code == 401 {
                ...
                throw GoCardlessError.apiError("Institution with ID \(id) not found")
            } else {
                let responseBuffer = try await response.body.collect(upTo: 1024 * 1024) // 1MB limit
                let responseBody = String(buffer: responseBuffer)
                throw GoCardlessError.apiError("HTTP \(response.status.code): \(responseBody)")
        ...
        let decoder = JSONDecoder()
        let institution = try decoder.decode(GoCardlessInstitution.self, from: Data(buffer.readableBytesView))
    ...
    func createEndUserAgreement(
        ...
        let body = try JSONSerialization.data(withJSONObject: payload)
        let request = try await createRequest(path: "/agreements/enduser/", method: .POST, body: body)
        ...
        let decoder = JSONDecoder()
        let agreement = try decoder.decode(GoCardlessEndUserAgreement.self, from: Data(buffer.readableBytesView))
    ...
    func getEndUserAgreement(id: String) async throws -> GoCardlessEndUserAgreement {
        ...
        let request = try await createRequest(path: "/agreements/enduser/\(id)/", method: .GET)
        let response = try await client.execute(request, timeout: .seconds(30))
        ...
        guard response.status.code == 200 else {
            if response.status.code == 401 {
                ...
                throw GoCardlessError.apiError("End User Agreement with ID \(id) not found")
            } else {
                let responseBuffer = try await response.body.collect(upTo: 1024 * 1024) // 1MB limit
                let responseBody = String(buffer: responseBuffer)
                throw GoCardlessError.apiError("HTTP \(response.status.code): \(responseBody)")
    ...
    func acceptEndUserAgreement(id: String, userAgent: String, ipAddress: String) async throws -> GoCardlessEndUserAgreement {
        try await ensureValidToken()
        ...
        let body = try JSONSerialization.data(withJSONObject: payload)
        let request = try await createRequest(path: "/agreements/enduser/\(id)/accept/", method: .PUT, body: body)
    ...
    func createRequisition(
        ...
        let body = try JSONSerialization.data(withJSONObject: payload)
        let request = try await createRequest(path: "/requisitions/", method: .POST, body: body)
        ...
        let decoder = JSONDecoder()
        let requisition = try decoder.decode(GoCardlessRequisition.self, from: Data(buffer.readableBytesView))
    ...
    func getRequisition(id: String) async throws -> GoCardlessRequisition {
        ...
        guard response.status.code == 200 else {
            if response.status.code == 401 {
                ...
                throw GoCardlessError.apiError("Requisition with ID \(id) not found")
            } else {
                let responseBuffer = try await response.body.collect(upTo: 1024 * 1024) // 1MB limit
                let responseBody = String(buffer: responseBuffer)
                throw GoCardlessError.apiError("HTTP \(response.status.code): \(responseBody)")
    ...
    func getAccountDetails(id: String) async throws -> GoCardlessAccount {
        ...
        let decoder = JSONDecoder()
        let account = try decoder.decode(GoCardlessAccount.self, from: Data(buffer.readableBytesView))
    ...
    func getAccountTransactions(
        ...
        let request = try await createRequest(path: "/accounts/\(id)/transactions/\(queryString)", method: .GET)
        let response = try await client.execute(request, timeout: .seconds(30))
        ...
        let decoder = JSONDecoder()
        let transactions = try decoder.decode(GoCardlessTransactionsResponse.self, from: Data(buffer.readableBytesView))
    ...
    private func createRequest(path: String, method: HTTPMethod, body: Data? = nil) async throws -> HTTPClientRequest {
