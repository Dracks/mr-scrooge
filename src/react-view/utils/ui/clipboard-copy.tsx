...
export const CopyStatusLine: React.FC<CopyStatusLineProps> = ({ status, successLabel = 'Copied!' }) => {
    switch (status.state) {
        case 'error':
            return <Text size="small" color="status-error">{status.message}</Text>;
        case 'success':
            return <Text size="small" color="status-ok">{successLabel}</Text>;
        default:
...
export function useClipboardCopy() {
    ...
    const [status, setStatus] = React.useState<CopyState>({ state: 'idle' });
    ...
    return { handleCopy, status, supported: isClipboardSupported };
}
