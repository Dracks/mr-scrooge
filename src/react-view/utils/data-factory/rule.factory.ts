...
export const ruleFactory : Factory.Factory<Rule> = baseRuleFactory.combine(Factory.Sync.makeFactory<Pick<Rule, 'labelIds'>>({
    labelIds: [],
}))

export const ruleEnhancedFactory = baseRuleFactory.combine(Factory.Sync.makeFactory<Pick<RuleEnriched, 'labels' | 'children'>>({
    children: [],
    labels: [],
}));
