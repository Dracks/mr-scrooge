...
const DemoPagination: React.FC<DemoPaginationProps> = ({ cb, options }) => {
    ...
    return <div>
        <button onClick={() => { pagination.start() }} data-testid="start-button">start</button>
        <button onClick={() => { pagination.reset(true) }} data-testid="reset-button">start</button>
        <ul>
            <li data-testid="status">{pagination.status }</li>
            <li><ul data-testid="data">
                {pagination.loadedData.map(data => <li key={data}>{data}</li>)}
            </ul>
            </li>
        </ul>
    </div>
