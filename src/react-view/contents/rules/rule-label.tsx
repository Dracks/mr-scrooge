...
export const RuleLabel: React.FC<{ rule: RuleEnriched, updateRule: (r: Rule)=>void }> = ({rule, updateRule}) => {
    ...
    const removeLabel = useAsyncCallback(async (ruleId: ApiUUID, labelId: ApiUUID) : Promise<void> =>{
        const response = await client.DELETE('/rules/{ruleId}/label/{labelId}', {params: {path: {ruleId, labelId}}})
        if (response.data){
            updateRule(response.data)
        }
    })
    ...
    <Box>
        <LabelInput
            ...
            onAdd={label => {
                ...
                catchAndLog( addLabel.execute(rule.id, label.id)
                , "Error adding a label to the rule", logger)
            }}
            onRemove={label => {
                setLabels(labels => labels.filter(test => test.id != label.id));
                catchAndLog(removeLabel.execute(rule.id, label.id), "Error removing a label from the rule", logger)
            }}
