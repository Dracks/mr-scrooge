        ...
        <Box direction='row-responsive' gap="small">
            <Box>
                ...
                <Box direction='row-responsive' >
                    <Tag name="labels" value={`${rule.labels.length}`} />
                    <Tag name="conditions" value={`${rule.conditions.length}`} />
                </Box>
            </Box>
            <Box justify='center'>
...
const RuleSubList: React.FC<{rules: RuleEnriched[]}> = ({rules})=>{
    return <Box pad="small">
        {rules.map(rule => <RuleSummary rule={rule} />)}
    </Box>
}

const AddRulePlaceholder : React.FC = ()=>{
    const navigate = useNavigate()
    return <Box pad="small" background="light-2" justify="center" align="center">
        <Button
