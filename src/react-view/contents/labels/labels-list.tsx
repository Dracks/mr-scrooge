...
export const LabelsList: React.FC = ()=>{
    ...
    return <Box direction="row" align='center' pad='small' gap='small'>
        {labels.map((label) => (<Tag key={label.id} name={groups.get(label.groupOwnerId)?.name ?? ''} value={label.name} align="center" />))}
    </Box>
}
