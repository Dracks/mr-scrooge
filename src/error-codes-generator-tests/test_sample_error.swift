...
final class DefaultData {
	...
	func testGenerateErrorCodes() {
		...
		#expect(output.contains("public enum ErrorCodes: String, CaseIterable, Sendable, ErrorMsg"))
	...
	func testGenerateErrorCodesWithAdditionalInfo() {
		let output = generateErrorCodes(from: [
			"E1002": .init(message: "Error with additional info", additionalInfo: "Check configuration")
		])
	...
	func testGenerateMultipleErrorCodes() {
		let output = generateErrorCodes(from: [
			...
			"E1003": .init(message: "Third error", additionalInfo: "Additional info for third"),
			"E1002": .init(message: "Second error", additionalInfo: "Additional info for second")
		])
