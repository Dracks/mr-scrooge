...
extension MrScroogeAPIImpl {
		...
		-> Operations.ApiLabels_create.Output
	{
        ...
        return try .ok(.init(body: .json(.init(label: label))))
	}
		...
		-> Operations.ApiLabels_list.Output
	{
        ...
        return try .ok(.init(body: .json(.init(results: data.list.map {try .init(label: $0)}, next: data.next))))
	}
}
...
extension Components.Schemas.Label {
    init(label: Label) throws {
        ...
        name = label.name
        groupOwnerId = try label.groupOwner.requireID().uuidString
    }
}
