...
extension MrScroogeAPIImpl {
		...
		-> Operations.ApiSession_updateMe.Output
	{
		...
		let updateUser: Operations.ApiSession_updateMe.Input.Body.jsonPayload
		switch input.body {
		case .json(let _updateUser):
			updateUser = _updateUser
		}

		guard let newDefaultGroupId = try await userGroupService.validateGroupId(on: request.db, groupId: updateUser.defaultGroupId, forUserId: user.requireID()) else {
			return .badRequest(.init(body: .json(.init(message: "Invalid default Group Id", code: ApiError.API10017.rawValue ))))
		}
