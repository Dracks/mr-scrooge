...
final class EnvConfig: Sendable {
	...
	private init() {
		...
		if let envAuthCodeExpiration = Environment.get("AUTHORIZATION_CODE_EXPIRATION_SECONDS") {
			authorizationCodeExpirationSeconds = TimeInterval(envAuthCodeExpiration) ?? 600  // Default to 10 minutes
		} else {
