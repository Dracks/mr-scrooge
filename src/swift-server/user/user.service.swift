...
class UserService {
	...
	func updateUser(on db: Database, userId: UUID, userData: User) async throws
		-> updateUserReturn
	{
...
class UserGroupService {
	func validateGroupId(on db: Database, groupId: String, forUserId userId: UUID) async throws -> UUID? {
	...
	func validateGroupId(on db: Database, groupId: UUID, forUserId userId: UUID) async throws -> Bool {
		let existsRelation = try await UserGroupPivot.query(on:db).filter(\.$group.$id == groupId).filter(\.$user.$id == userId).count()
