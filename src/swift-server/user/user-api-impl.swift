...
extension MrScroogeAPIImpl {
		...
		-> Operations.ApiUser_update.Output
	{
		...
		guard let newDefaultGroupId = try await userGroupService.validateGroupId(on: request.db, groupId: updateUser.defaultGroupId, forUserId: userId) else {
			return .badRequest(.init(body: .json(.init(message: "Invalid default Group Id", code: ApiError.API10018.rawValue ))))
		}
