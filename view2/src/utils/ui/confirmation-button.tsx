...
interface ConfirmationButtonArgs extends Omit<DropButtonExtendedProps, 'onOpen' | 'onClose' | 'open' | 'dropContent' | 'dropProps'>{
    onConfirm: ()=>void
...
export const ConfirmationButton : React.FC<ConfirmationButtonArgs> = ({onConfirm, confirmationText, ...props})=>{
    ...
    return <DropButton 
        ...
        dropContent={<Box pad="small" round gap="small" background={{ color: 'light-2', opacity: 'strong' }}>
            <Text>
                {confirmationText? confirmationText : "Are you sure?" }
            </Text>
            <Button label='yes' primary color={props.color} onClick={()=>{
