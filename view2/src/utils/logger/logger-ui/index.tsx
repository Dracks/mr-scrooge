...
export const LoggerUi : React.FC<{}> = ()=>{
    ...
    loggerEmitter.on(LogLevel.info, ev => setLogs([ev, ...logs]))
    loggerEmitter.on(LogLevel.warn, ev => setLogs([ev, ...logs]))
    loggerEmitter.on(LogLevel.error, ev => setLogs([ev, ...logs]))
    ...
    return <div style={{position: "absolute",
        ...
        border: "1px solid #ccc"
            ...
            {showLogs ?
            <ul>
                {logs.map(event => <li key={event.ts.getTime()}>{event.ts.toISOString()} [{event.level}]: {event.msg} {event.data ? <RenderUnknown data={event.data}/> : undefined}</li>)}
            </ul>
            : undefined}
