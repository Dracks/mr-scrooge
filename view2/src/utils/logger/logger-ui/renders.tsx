...
const RenderArray: React.FC<{data: unknown[]}> = ({data}) =><ul> "array({data.length})"
</ul>

const RenderObject: React.FC<{data: object}> = ({data}) => {
    const [show, setShow] = React.useState<boolean>(false)
    return <ul> "object" <button onClick={()=>setShow(!show)}>show/hide</button>
        {show? Object.entries(data).map(([key, elem], idx) => <li key={key}>
