...
import {
    graphToUi,
    GraphUiRepresentation,
    uiToGraph,
} from "./graph.transformer";
...
export const GraphForm: <T extends Partial<GraphV2>>(
    p: GraphFormProps<T>
) => React.ReactElement<GraphFormProps<T>> = <T extends Partial<GraphV2>>({
    graphData,
    update,
    save,
}: GraphFormProps<T>) => {
    ...
    const hasHorizontal =
        graphData.kind === GraphKind.bar || graphData.kind === GraphKind.line;
    const graphEnabled =
        (graphData.kind && graphData.group && !hasHorizontal) ||
        graphData.horizontalGroup;
    ...
    return (
        ...
        >
            <Box direction={size === "small" ? "column" : "row"} width="fill">
                {graphEnabled ? (
                    <GraphViewer
                        graph={enrichGraph(graphData as GraphV2, tags)}
                    />
                ) : (
                    <GraphPlaceholder />
                )}
                <Box>
                    ...
                    <Box>
                        ...
                        {graphUi.groupKind === GraphGroupEnum.tags && (
                            <React.Fragment>
                                <FormField
                                    label="Tags to group"
                                    htmlFor="select-group-tags"
                                >
                                    <InputTag
                                        value={
                                            graphUi.groupTags?.map(
                                                (tagId) => tagsMap[tagId]
                                            ) ?? []
                                        }
                                        onAdd={(tag) =>
                    ...
                    {hasHorizontal && (
                        <Box>
                            ...
                            {graphUi.horizontalGroupKind ===
                                GraphGroupEnum.tags && (
                                <React.Fragment>
                                    <FormField
                                        label="Tags to group"
                                        htmlFor="select-x-group-tags"
                                    >
                                        <InputTag
                                            value={
                                                graphUi.horizontalGroupTags?.map(
                                                    (tagId) => tagsMap[tagId]
                                                ) ?? []
                                            }
                                            onAdd={(tag) =>
