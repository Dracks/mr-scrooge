...
export const createGroup =<K extends string>(data: DTInputData[], lambda: {callback: DTGroupFn<K>, name: string}): GenericDSGroup<K, DTInputData[]>[] => {
...
export const createGroupWithSubGroup = <K extends string, SK extends string>(data: DTInputData[], lambda: {name: string, callback:DTGroupFn<K>}, subLambda: {name: string, callback: DTGroupFn<SK>}): GenericDSGroup<K, GenericDSGroup<SK, DTInputData[]>[]>[] => {
