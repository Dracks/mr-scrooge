...
export const Graphs: React.FC = () => {
    ...
    const joinedGraphsResponse = useJoinedGraphs(
        graphsV2Response,
        graphsResponse
    );
    const responseGraphList = joinedGraphsResponse.data;
    const oldGraphs = responseGraphList?.filter(
        (graph) => !graph.id && graph.oldGraph
    );
    React.useEffect(() => {
    ...
    if (joinedGraphsResponse.loading) {
        ...
        const graphs = responseGraphList.map((graph) =>
            enrichGraph(graph, tags)
        );
        return (
