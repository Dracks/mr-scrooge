...
const getGroupByLambda = <T extends string | number>(
    data: DMData[],
    lambda: DMGroupFn<T>
): Record<T, DMData[]> => {
...
class DataManagerDoubleGrouped<
...
> {
    ...
    public reduceGroups(callback: DMReduceFn) {
        ...
        (Object.keys(data) as T1[]).forEach((keyFirst) => {
            ...
            newData[keyFirst] = (Object.keys(subGroup) as T2[]).reduce(
                (acc, keySecond) => {
...
export class DataManage {
    ...
    public groupForGraph<
        T1 extends string | number,
        T2 extends string | number
    >(firstLambda: DMGroupFn<T1>, secondLambda: DMGroupFn<T2>) {
