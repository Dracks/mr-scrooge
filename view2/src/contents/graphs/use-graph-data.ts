...
export const useGraphDataGenerator = ({
    tagFilter,
    dateRange,
    horizontalGroup,
    group,
    ...
    let rdsList = tagFilter
        ? data.filter((rds) => rds.tags.indexOf(tagFilter) >= 0)
        : data;
    rdsList = monthRange
        ? rdsList.filter(getRangeFilter(monthRange, new Date()))
        : rdsList;

    const groupLambda = groupLambdas[group.group](
        group.groupTags,
        group.hideOthers ?? false
    );
    const horizontalGroupLambda = horizontalGroup
        ? groupLambdas[horizontalGroup.group](
              horizontalGroup.groupTags,
              horizontalGroup.hideOthers ?? false
          )
    ...
    const sortLambda = horizontalGroup
        ? sortLambdas[horizontalGroup.group](
              horizontalGroup.groupTags.map(tagMap)
          )
