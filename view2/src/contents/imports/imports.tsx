...
const ImportDetailsSwitcher: React.FC<{ importsList: StatusReport[] }> = ({
    importsList,
}) => {
    const { id } = useParams();
    const status = importsList.find(
        (status) => status.id === Number.parseInt(id ?? "NaN", 10)
    );
            ...
            <Box fill>
                <Routes>
                    ...
                    <Route
                        path=":id"
                        element={
                            <ImportDetailsSwitcher importsList={importsList} />
                        }
                    />
                </Routes>
            </Box>
