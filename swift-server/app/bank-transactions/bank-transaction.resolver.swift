...
extension MrScroogeResolver {
	...
	func bankTransaction(req: Request, arguments: BankTransactionTypes.GetBankTransactionArgs)
		async throws -> BankTransactionTypes.GetBankTransactionsResponse
	{
		...
		let data = try await BankTransactionTypes.bankTransactionService.getAll(
			on: req.db, groupIds: groupIds, cursor: arguments.cursor,
