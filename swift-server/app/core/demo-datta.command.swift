...
struct DemoDataCommand: Command {
    ...
    private func generateTagAndTransactions(app: Application, groupOwnerId: UUID, label: String, amountRange: ClosedRange<Double>, periodicity: String) throws -> UUID {
        ...
        while date > finishDate {
            ...
            let labelTransaction = LabelTransaction(labelId: label.id!, transactionId: transaction.id!)
            try labelTransaction.save(on: app.db).wait()

            if periodicity == "days" {
                date = calendar.date(byAdding: .day, value: -Int.random(in: 1...10), to: date)!
            } else {
                date = calendar.date(byAdding: .month, value: -1, to: date)!
                date = calendar.date(byAdding: .day, value: Int.random(in: -1...1), to: date)!
            }
        }
    ...
    private func generateGraphs(app: Application, groupOwnerId: UUID, labelIdMap: [String: UUID]) throws {
        ...
        if let groceriesId = labelIdMap["groceries"], let gasolineId = labelIdMap["gasoline"] {
            let groceriesLabel = GraphGroupLabels(graphId: compareLabelsGroup.id!, labelId: groceriesId)
            try groceriesLabel.save(on: app.db).wait()
            
            let gasolineLabel = GraphGroupLabels(graphId: compareLabelsGroup.id!, labelId: gasolineId)
