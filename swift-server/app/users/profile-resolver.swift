...
extension MrScroogeResolver {
	func login(request req: Request, arguments: SessionTypes.LoginArgs) async throws
		-> SessionTypes.LoginResponse
	{
		var errorStr = ""
	...
	func getCurrentUser(request req: Request, arguments: NoArguments) async throws
		-> SessionTypes.MyProfileResponse
	{
		do {
	...
	func users(request req: Request, arguments: NoArguments) async throws -> [SessionTypes
		.MyProfile]
	{
		// todo, validate it's only for admins
		return try await User.query(on: req.db).all().map({
			return SessionTypes.MyProfile(user: $0)
		})
	}
...
class SessionTypes {
	...
	class Schema: PartialSchema<MrScroogeResolver, Request> {
		...
		override var query: Fields {
			Field(
				"me", at: MrScroogeResolver.getCurrentUser,
				as: MyProfileResponse.self)
...
class UsersService {
	func getGroups(on db: Database, forUser userId: UUID) async throws -> [UserGroup] {
		return try await User.query(on: db).filter(\.$id == userId).with(\.$groups).all()
			.flatMap { user in
				user.groups
			}
	}
