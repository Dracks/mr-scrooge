...
class TransformHelper<T: Hashable> {
    let mapping: FieldsMap<T>
    let dateFormatter: DateFormatter
    
    init(_ mapping: FieldsMap<T>, dateFormat: String ) {
        self.mapping = mapping
        dateFormatter = DateFormatter()
        dateFormatter.dateFormat = dateFormat
    }
    
    func map(_ row: [T: Any]) throws -> PartialBankTransaction {
        ...
        if let name = row[mapping.movementName] as? String {
            movementName = name
        } else {
            errorFields.append("movementName")
        }
        
        if let dateString = row[mapping.date] as? String,
        ...
        if let parsedValue = parseToDouble(row[mapping.value]) {
            value = parsedValue
        } else {
        ...
        if let dateValueKey = mapping.dateValue {
            if let dateValueString = row[dateValueKey] as? String,
        ...
        if let detailsKey = mapping.details {
            if let parsedDetails = row[detailsKey] as? String {
                details = parsedDetails
            }
        }
        
        if let descriptionKey = mapping.description {
            if let parsedDescription = row[descriptionKey] as? String {
