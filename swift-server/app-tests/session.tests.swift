...
final class SessionTests: AbstractBaseTestsClass {
    ...
    func testMyProfileQueryWithAuthenticatedRequest() async throws {
        ...
        let res = try await app.queryGql(GraphQLRequest(query: query), headers: identifiedHeaders)
        XCTAssertEqual(res.status, .ok)
        let data = try res.content.decode(GraphQLResult.self)
    ...
    func testValidLoginMutation() async throws {
        ...
        let res = try await app.queryGql(GraphQLRequest(query: loginMutation, variables: toVars(["username": testUser.username, "password": "test-password"])))
        XCTAssertEqual(res.status, .ok)
        let data = try res.content.decode(GraphQLResult.self)
        XCTAssertEqual(data.errors, [])
    ...
    func testInvalidLoginMutation() async throws {
        ...
        let res = try await app.queryGql(GraphQLRequest(query: loginMutation, variables: toVars(["username": testUser.username, "password": "wrong-password"])))
        XCTAssertEqual(res.status, .ok)
        let data = try res.content.decode(GraphQLResult.self)
